...
async function sendMail({ to, subject, html, from, replyTo }) {
  ...
  return mailer.send({
    ...
    from: from || {
      ...
      name: process.env.SENDGRID_FROM_NAME || "SignLink Support",
    },
...
exports.sendProposalEmail = async (req, res) => {
  try {
    ...
    // ✅ 3. Create parent ProposalEmail record
    const proposalEmail = await db.models.ProposalEmail.create({
      headerId,
      parentId,
      userId,
    ...
    // ✅ 5. Background process starts (non-blocking) using SendGrid
    (async () => {
      try {
        let results = [];
...
exports.updateExpirationDate = async (req, res) => {
  try {
    ...
    // 3. Send update email to each recipient using SendGrid
    for (const recipient of recipients) {
      if (recipient.status !== "sent" && recipient.status !== "opened")
        continue;
...
`;
      ...
      try {
        ...
        results.push({ email: recipient.recipientEmail, status: "notified" });
        console.log(`✅ Update email sent to: ${recipient.recipientEmail}`);
      } catch (err) {
        console.error(
          "Failed to send update email to",
          recipient.recipientEmail,
...
exports.sendReminder = async (req, res) => {
  try {
    ...
    // ✅ Create individual recipient objects
    const emailArray = clientEmails.split(',').map(email => email.trim());
    const nameArray = clientNames.split(',').map(name => name.trim());

    const recipients = emailArray.map((email, index) => ({
      email: email,
      name: nameArray[index] || "there"
    }));
    ...
    // ✅ Background process starts (non-blocking) using SendGrid
    (async () => {
      try {
        let results = [];
